...
def get_dense_logits2(image_features, local_image_features, all_text_features, mean_text_features, topk=50, global_weight=0.1):
    ...
    # Applied Global Weight to perfectly balance global vs local
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, cache_keys=None, cache_values=None):
        ...
        # ---------------- STUDENT LESION TIP-ADAPTER-F MEMORY CACHE ----------------
        ...
        if cache_keys is not None:
            ...
            # Learnable class-specific valve (alpha) and sharpness (beta)
    ...
    def forward(self, image, mask=None, labels=None):
        ...
        g_weight = getattr(self, "global_weight", 0.1)
        ...
        # ---------------- IMPROVED: STUDENT-SIDE LESION TIP-ADAPTER WITH SIGMOID GATING ----------------
        if getattr(self, "tip_adapter", None) is not None:
            text_tea = self.text_features_tea.to(local_image_features.device).to(self.dtype)
            
            # STUDENT-SIDE: Use the Student's local_image_features to find the lesion
            ...
            # Keep beta safely positive during training
            ...
            # Scale cache up to CLIP's math space, then gate it safely
            scaled_cache_logits = (cache_logits * logit_scale) * gate
            
            # We ONLY add the cache to the final logits so it doesn't corrupt the spatial representation learning
...
class LocProto(TrainerX):
    ...
    def build_model(self):
        ...
        # ---------------- CONSTRUCT STUDENT LESION-ONLY TIP-ADAPTER CACHE ----------------
        print("Extracting Pristine Lesion-Only Cache from training set...")
        ...
        with torch.no_grad():
            # Generate baseline text to find the lesions
            ...
            for batch in tqdm(cache_loader, desc="Building Lesion Cache"):
                ...
                # Extract local features to find lesions (Using frozen backbone at initialization)
                _, local_feat, _ = clip_model.visual(image.type(clip_model.dtype))
                local_feat = F.normalize(local_feat, p=2, dim=-1)
                
                # Find exact lesion patches using ground truth text
                ...
                # Average the top-k lesion patches
        ...
        cache_values = F.one_hot(cache_labels, num_classes=len(classnames)).float().to(self.device).to(clip_model.dtype) 
        # ---------------------------------------------------------------------------------
        ...
        if "ViT" in cfg.MODEL.BACKBONE.NAME:
            ...
            if hasattr(self.model, "tip_adapter") and self.model.tip_adapter is not None:
                ...
                # Pass ALL new parameters to the optimizer!
                tip_params =[
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            # Zero out ALL registered optimizers safely
    ...
    def load_model(self, directory, epoch=None):
        ...
        for name in names:
            ...
            # Allow dynamic shape matching for tip_adapter in case we switch datasets
